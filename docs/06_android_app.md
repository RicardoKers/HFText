...
The Compose UI separates normal field operation from a deliberately small Settings screen. Operation keeps the chat-style TX/RX history, compact RX waterfall, symbol/duration/TX-progress line, message composer, and a compact Fast/Slow selector below the composer. Changing Fast/Slow while capture is active replaces the native streaming receiver immediately and clears queued decoder blocks, while keeping `AudioRecord`, waterfall, and evidence capture running. The title, navigation, Clear, Send/Stop, Fast, and Slow controls use reduced heights to preserve vertical space. Settings contains only callsign, `Pause RX during TX`, Start/Stop RX, Save RX evidence, and Reset local settings. The Android message panel keeps a scrollable recent history of up to 100 timestamped TX/RX messages. A compact `Sender` menu beside Clear lists callsigns extracted from current RX history; a selected sender gives matching RX bubbles a muted amber background while all other traffic remains visible. The sender selection and local operator state, including callsign, draft message, Fast/Slow profile, RX-during-TX pause state, and recent message history, are restored from app-private preferences on launch. Settings can reset the local operator settings without clearing messages or saved evidence files; reset uses `nocall` as a generic callsign placeholder. The screen is kept awake while TX or RX is active to avoid interrupting long field captures.
...
- The Android package version is aligned with the shared HFText 0.7.0 application version.
...
- Changing Fast/Slow during active Android capture recreates the native receiver with the selected profile, clears pending decoder audio, and preserves microphone/evidence capture.
...
- Android can highlight one sender selected from callsigns present in the current
  RX history; matching is case-insensitive against the first payload token,
  does not hide other traffic, and does not alter TX bubbles.
