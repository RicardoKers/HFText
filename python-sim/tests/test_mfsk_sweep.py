...
def test_run_mfsk_sweep_compares_2fsk_4fsk_and_8fsk(tmp_path):
    ...
    assert modes == {"2fsk-v0.1", "4fsk-v0.2-exp", "8fsk-v0.3-exp"}
    ...
    clean_8fsk = next(result for result in aggregates if result.mode == "8fsk-v0.3-exp" and result.snr_db is None)
    assert clean_4fsk.duration_s < clean_2fsk.duration_s
    assert clean_8fsk.duration_s < clean_4fsk.duration_s
