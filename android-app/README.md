# HFText Android App

This directory contains the first Android shell for HFText.

Current status:

- Gradle wrapper project;
- Android package version aligned with the shared HFText 0.7.0 application version;
- Kotlin source enabled through Android Gradle Plugin built-in Kotlin support;
- Jetpack Compose UI shell;
- adaptive and legacy launcher icons derived from the PC HFText artwork;
- minimal JNI bridge to the portable C ABI;
- native metadata and Fast/Slow profiles used through the JNI bridge;
- native text preparation and TX estimates shown while editing;
- native tone-frequency lists used by the waterfall and saved in evidence reports;
- compact Operation/Settings screen split so field use is less cluttered;
- Operation shows timestamped TX/RX history, a lightweight RX waterfall, TX
  symbol/duration estimate, live TX progress, compact message actions, and a
  compact Fast/Slow selector below the composer;
- Settings holds only callsign, optional RX-during-TX pause, Start/Stop RX, Save
  RX evidence, and Reset local settings;
- callsign, draft message, Fast/Slow profile, and RX-during-TX pause state are
  persisted locally between app launches;
- changing Fast/Slow during active capture switches the native RX profile without
  stopping microphone, waterfall, or evidence capture;
- local operator settings can be reset from Settings without clearing message
  history or evidence files; reset uses `nocall` as a generic callsign placeholder;
- up to 100 recent TX/RX messages are shown in a scrollable timestamped panel
  and persisted locally between app launches;
- RX messages can be highlighted by a sender selected from callsigns already
  present in the local history without hiding other traffic;
- the screen is kept awake while TX or RX is active;
- explicit AudioTrack TX using audio generated by the portable core;
- AudioRecord RX capture with native audio level/clipping statistics;
- AudioRecord RX blocks fed to the native streaming receiver;
- optional half-duplex decoder pause during TX while AudioRecord, waterfall,
  and evidence capture continue;
- accepted RX messages and explicit TX messages shown in a timestamped chat-style history, plus receiver status
  summarized through the shared C ABI RX-event helper;
- Settings keeps the instantaneous decoder state separate from the stable
  last accepted message so post-frame noise does not hide the latest success;
- Android RX requests voice-recognition input and falls back internally when the
  source is unavailable; normal Settings does not expose an audio-source choice;
- Android RX applies a limited digital gain before the native receiver and shows
  raw/modem peak levels for diagnosis;
- low-confidence receiver activity is counted for field diagnosis;
- Android RX can save the recent 240 s raw and modem-input audio buffers as WAV files
  plus a text evidence report under the app-specific `rx-evidence` directory for
  PC-side replay and field review;
- Android RX shows the captured buffer duration and warns when saved evidence is
  shorter than the selected TX estimate;
- Android RX capture runs independently from native receiver processing so the
  recorded evidence duration should advance in real time even if decoding lags;
- no modem logic duplicated in Kotlin.

Android development should continue to use:

- Kotlin and Jetpack Compose for the UI;
- `AudioTrack` for TX audio;
- `AudioRecord` for RX audio;
- JNI as a narrow bridge to the portable C ABI in `core/include/hftext_c_api.h`;
- the existing C++ core for modem settings, TX helpers, tone lists, audio statistics, and streaming RX.

Do not duplicate modem protocol, text encoding, FEC, modulation, or receiver logic in Kotlin.

The C ABI usage contract for JNI work is documented in:

```text
docs/12_c_api_reference.md
```

The current JNI bridge is intentionally small. It loads:

- `libhftext_c_api.so`;
- `libhftext_android_jni.so`.

Kotlin reads application metadata, protocol metadata, default Fast/Slow profile
summaries, sanitized text, payload preview, symbol counts, TX estimates, generated
TX audio, RX sample rate, tone frequencies, audio statistics, and streaming RX
updates/event summaries through this native path. Audio playback starts only after the operator presses `Send`.
RX capture starts only after the operator grants microphone permission and presses
`Start RX capture` in Settings. Captured audio is streamed to the portable receiver; accepted
messages are displayed only after core-side frame, payload, and CRC validation.
`Save RX evidence` writes raw/modem WAV files and a TXT report with metadata, RX
counters, instantaneous decoder state, stable last accepted message, elapsed
time from the last accepted message to evidence save, TX/RX message history, the
active RX profile for each accepted frame, and the core-reported acceptance
latency when available.

## Build

From the repository root:

```powershell
.\scripts\build_android_debug.ps1
```

Install the debug APK on a connected emulator or Android device:

```powershell
.\scripts\install_android_debug.ps1
```

Useful variants:

```powershell
.\scripts\install_android_debug.ps1 -ListDevices
.\scripts\install_android_debug.ps1 -DeviceId <adb-device-id>
.\scripts\install_android_debug.ps1 -SkipBuild
.\scripts\install_android_debug.ps1 -NoLaunch
```

Or directly from this directory, when `JAVA_HOME` and `ANDROID_HOME` are already set:

```powershell
.\gradlew.bat assembleDebug
```

The generated debug APK is written under:

```text
android-app/app/build/outputs/apk/debug/
```

Open `android-app/` in Android Studio to run it on an emulator or device.

## Environment Check

From the repository root:

```powershell
.\scripts\check_android_environment.ps1
```

The script only reports installed tools, detected versions, and practical warnings. It does not install or modify anything.

For Windows installation steps, see:

```text
docs/11_android_windows_setup.md
```

Use strict mode when the Android build becomes part of validation:

```powershell
.\scripts\check_android_environment.ps1 -Strict
```
