...
O app PC passou a mostrar uma linha `Estado RX` na aba de operacao. Ela resume os eventos do `StreamingReceiver` em estado recente, ultimo `PHYS_LENGTH`, qualidade do ultimo candidato completo e ultimo motivo de rejeicao, ajudando testes em campo sem exigir log detalhado ligado o tempo todo.

O app PC tambem passou a mostrar `Sessao RX`, um resumo com contadores reiniciados a cada nova recepcao: quadros aceitos, candidatos rejeitados, `PHYS_LENGTH` recuperados e candidatos de sync. O resumo tambem entra no cabecalho dos logs/evidencias exportados.
